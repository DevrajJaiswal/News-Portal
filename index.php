            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                    ...
                    $sql = "select post.post_id, post.title, post.description, post.post_date, post.post_img, post.author, 
                            category.category_id, category.category_name, user.first_name, user.last_name from post 
                            left join category on post.category= category.category_id 
                            left join user on post.author = user.user_id order by post.post_date desc limit {$offset},{$page_limit}";
                                        ...
                                        <div class="inner-content clearfix">
                                            ...
                                            <div class="post-information">
                                                ...
                                                <span>
                                                    ...
                                                    <a href='author.php?auth_id=<?php echo $row['author']; ?>'><?php echo $row['first_name']." ".$row['last_name']; ?></a>   <!-- in link author.php -->
                                                </span>
                                                <span>
                    ...
                    if ($rows > $page_limit) {
                        ...
                        if ($page > 1) {
                            $pre = $page - 1;
                            echo "<li><a href='index.php?page={$pre}'>Prev</a></li>";
                        }
                        while ($i <= $total_page) {
                            if ($i == $page) {
                                $is_active = "active";
                            ...
                            echo "<li class='{$is_active}'><a href='index.php?page={$i}'>{$i}</a></li>";

                            $i++;
                        }
                        if ($total_page > $page) {
                            $nex = $page + 1;
                            echo "<li><a href='index.php?page={$nex}'>Next</a></li>";
                        }
                        echo "</ul>";
                    }
                    ?>
