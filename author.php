            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                    ...
                    $sql2 = "select post.post_id, post.title, post.description, post.post_date, post.post_img, category.category_id, category.category_name, user.first_name, user.last_name from post 
                            ...
                            left join user on post.author = user.user_id where user_id='{$auth_id}' order by post.post_date desc limit {$offset},{$page_limit}";
                    ...
                    <?php
                    // Pagination code
                    $sql3 = "select * from post where author='{$auth_id}'";
                    ...
                    if ($rows > $page_limit) {
                        ...
                        if ($page > 1) {
                            $pre = $page - 1;
                            echo "<li><a href='author.php?auth_id={$auth_id}&page={$pre}'>Prev</a></li>";
                        }
                        while ($i <= $total_page) {
                            if ($i == $page) {
                                $is_active = "active";
                            ...
                            echo "<li class='{$is_active}'><a href='author.php?auth_id={$auth_id}&page={$i}'>{$i}</a></li>";

                            $i++;
                        }
                        if ($total_page > $page) {
                            $nex = $page + 1;
                            echo "<li><a href='author.php?auth_id={$auth_id}&page={$nex}'>Next</a></li>";
                        }
                        echo "</ul>";
                    }
                    ?>
